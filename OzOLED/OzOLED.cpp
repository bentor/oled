...
void OzOLED::printIndicator(byte level, byte X, byte Y){
    ...
    for(byte i=0; i<8; i++) {
       ...
       sendData(pgm_read_byte(&Indicator[level][i]));
    }
}
...
void OzOLED::printString(const __FlashStringHelper *String, byte X, byte Y, byte numChar){
	...
	byte count=0;
    while(pgm_read_byte_near(((char *)String)+count) && count<numChar){
		printChar(pgm_read_byte_near(((char *)String)+count++));  
	}
...
void OzOLED::init(){
    ...
    // Bent G christensen <bentor@gmail.com>, May 2016
    // Important fix for some oled screen to turn on in the first place.
